...
COMPANY_NAMES = st.text_input("Enter Company Name")
st.text("To enter multiple variations of company names, separate them with comma (,)")
st.text("Example: Apple, Apple Inc, 苹果")

uploaded_file = st.file_uploader("Upload your headlines in Excel format here", type=['xlsx'])

summarize_button = st.button('Summarize Headlines')

if st.session_state.get('button') != True:
    st.session_state['button'] = summarize_button
...
if st.session_state.get('button') == True:
    if PIN == st.secrets["PIN"]:
        ...
        if uploaded_file is None or COMPANY_NAMES == "":
            ...
            try:
                ...
                # Count headlines that start with company name
                count_headlines = {}
                for i in COMPANY_NAMES.split(","):
                    company_name = i.strip()
                    if company_name != "":
                        # Count headlines that start with company name
                        count_headlines[company_name] = len(df[df['Headline'].str.startswith(company_name)])

                # Display Results
                st.text("Number of headlines that start with company name")
                st.dataframe(pd.DataFrame.from_dict(count_headlines, orient='index', columns=['Count']).sort_values(by=['Count'], ascending=False))
                CHOOSED_COMPANY_NAME = st.selectbox("Choose company name to be summarized", list(count_headlines.keys()))
                ...
                system_prompt_summary = f'''
                    ...
                    As an AI trained by OpenAI, your task is to summarize the daily news headlines concerning a specific company, named '{CHOOSED_COMPANY_NAME}'. 
                ...
                user_prompt_summary = f'''
                    ...
                    Summarize this headlines into a few bullet points talking about what is going on with '{CHOOSED_COMPANY_NAME}' right now in english. 
                ...
                if st.button("Summarize", type="secondary"):
                    st.divider()

                    # Filter headlines that start with company name
                    df = df[df['Headline'].str.startswith(CHOOSED_COMPANY_NAME)]
